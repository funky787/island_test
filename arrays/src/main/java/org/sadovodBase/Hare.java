...
public class Hare extends Herbivore {
    public Hare(int x, int y, Island island) {
        super(x, y, island);
    }
    ...
    protected void eat() {
        ...
        if (!loc.getPlants().isEmpty()) {
            ...
            System.out.printf("Заяц на (%d,%d) съел растение%n", x, y);
    ...
    protected void reproduce() {
        ...
        for (Animal a : new ArrayList<>(loc.getAnimals())) {
            if (a instanceof Hare && a != this && a.isAlive()){
                count++;
            }
        }
        if (count > 0 && Math.random() < 0.2) {
            Hare baby = new Hare(x, y, island);
            loc.addAnimal(baby);
            System.out.printf("На (%d,%d) родился Заяц%n", x, y);
        }
    }
}
