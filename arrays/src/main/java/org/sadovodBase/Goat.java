...
public class Goat extends Herbivore{
    ...
    protected void eat(){
        ...
        if (!loc.getPlants().isEmpty()){
            ...
            System.out.printf("Коза на (%d,%d) съел растение%n", x, y);
    ...
    protected void reproduce() {
        ...
        for (Animal a : new ArrayList<>(loc.getAnimals())) {
            if (a instanceof Goat && a != this && a.isAlive()){
                count++;
            }
        }
        if (count > 0 && Math.random() < 0.2) {
            Goat baby = new Goat(x, y, island);
            loc.addAnimal(baby);
            System.out.printf("На (%d,%d) родилась Коза%n", x, y);
        }
    }
}
