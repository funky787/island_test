...
public class Horse extends Herbivore{
    ...
    protected void eat(){
        ...
        if (!loc.getPlants().isEmpty()){
            ...
            System.out.printf("Лошадь на (%d,%d) съел растение%n", x, y);
    ...
    protected void reproduce() {
        ...
        for (Animal a : new ArrayList<>(loc.getAnimals())) {
            if (a instanceof Horse && a != this && a.isAlive()){
                count++;
            }
        }
        if (count > 0 && Math.random() < 0.2) {
            Horse baby = new Horse(x, y, island);
            loc.addAnimal(baby);
            System.out.printf("На (%d,%d) родилась Лошадь%n", x, y);
        }
    }
}
